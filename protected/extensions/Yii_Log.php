	...
	**/
    ...
    public static function getInstance($app = null,$logType=null)
    {
        ...
        if(empty(self::$arrInstance[$app]))
        {
            ...
            self::$arrInstance[$app] = new Yii_Log($log_conf);
        }
...
*/
    ...
    public function parseFormat($format) {
        ...
        for($i = 0; $i < $len; $i++) {
            ...
            switch($code) {
            case 'h':
                $action[] = "(defined('CLIENT_IP')? CLIENT_IP : Yii_Log::getClientIp())";
                break;
            case 't':
            ...
            case 'e':
                ...
                break;
            case 'f':
                $action[] = 'Yii_Log::$current_instance->current_file';
                break;
            case 'H':
            ...
            case 'L':
                $action[] = 'Yii_Log::$current_instance->current_log_level';
                break;
            case 'N':
                $action[] = 'Yii_Log::$current_instance->current_line';
                break;
            case 'E':
                $action[] = 'Yii_Log::$current_instance->current_err_no';
                break;
            case 'l':
                $action[] = "Yii_Log::genLogID()";
                break;
            case 'u':
                if (!isset($prelim_done['user'])) {
                    $prelim[] = '$____user____ = AccountService::$current_user;';
                    $prelim_done['user'] = true;
                }
                $action[] = "((defined('CLIENT_IP') ? CLIENT_IP: Yii_Log::getClientIp()) . ' ' . \$____user____['user_id']. ' ' . \$____user____['user_name'])";
                break;
            case 'S':
                if ($param == '') {
                    $action[] = 'Yii_Log::$current_instance->get_str_args()';
                } else {
                    $param_name = var_export($param, true);
                    if (!isset($prelim_done['S_'.$param_name])) {
                        $prelim[] = 
                            "if (isset(Yii_Log::\$current_instance->current_args[$param_name])) {
                            \$____curargs____[$param_name] = Yii_Log::\$current_instance->current_args[$param_name];
                            unset(Yii_Log::\$current_instance->current_args[$param_name]);
                ...
                break;
            case 'M':
                $action[] = 'Yii_Log::$current_instance->current_err_msg';
                break;
            case 'x':
                ...
                case 'err_no':
                case 'err_msg':
                    $action[] = 'Yii_Log::$current_instance->current_'.$param;
                    break;
                case 'log_id':
                    $action[] = "Yii_Log::genLogID()";
                    break;
                case 'app':
                    $action[] = "Yii_Log::getLogPrefix()";
                    break;
                case 'function_param':
                    $action[] = 'Yii_Log::flattenArgs(Yii_Log::$current_instance->current_function_param)';
                    break;
                case 'argv':
                    $action[] = '(isset($GLOBALS["argv"])? Yii_Log::flattenArgs($GLOBALS["argv"]) : \'\')';
                    break;
                case 'pid':
                    $action[] = 'posix_getpid()';
                    break;
                case 'encoded_str_array':
                    $action[] = 'Yii_Log::$current_instance->get_str_args_std()';
                    break;
                default:
                    $action[] = "''";
                }
    ...
    /**
     * 注册一个新的logWriter
     * @param Yii_Log_Writer $writer
     */
    private static function registerWriter(Yii_Log_Writer $writer){
    ...
    public static function useLogerStdOut(){
      ...
      self::registerWriter(new Yii_Log_Writer_Std());
      $used = true; 
    }
}
